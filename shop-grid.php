        			...
        			<div class="col-lg-9 col-12 order-1 order-lg-2">
						...
						<?php
							...
							if($action == "Manage"){
								...
								if(isset($_GET['meta_tag'])){
									$meta_tag = $_GET['meta_tag'];
									?>
										<div class="row">
											<div class="col-lg-12">
												<div class="shop__list__wrapper d-flex flex-wrap flex-md-nowrap justify-content-between">
										...
										<div class="tab__container">
											<div class="shop-grid tab-pane fade show active" id="nav-grid" role="tabpanel">
												<div class="row">
													<?php
														$booksCountsCat = $db->query("SELECT * FROM books INNER JOIN authors ON books.author_id = authors.a_id WHERE books.bk_meta LIKE '%$meta_tag%'")->num_rows;
														
														$getSubCat = "SELECT * FROM books INNER JOIN authors ON books.author_id = authors.a_id WHERE books.bk_meta LIKE '%$meta_tag%'";
														$resSubCat = mysqli_query($db, $getSubCat);
														if(mysqli_num_rows($resSubCat) > 0 ){
															while($rowSubCat = mysqli_fetch_assoc($resSubCat)){
																// $sub_catId     = $rowSubCat['cat_id'];
																$total_rowsCat = $db->query("SELECT * FROM books INNER JOIN authors ON books.author_id = authors.a_id WHERE books.bk_meta LIKE '%$meta_tag%'")->num_rows;

																$current_pageCat = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
									
																$rows_per_pageCat = 20;
									
																if($statementCat = $db->prepare("SELECT * FROM books INNER JOIN authors ON books.author_id = authors.a_id WHERE books.bk_meta LIKE '%$meta_tag%' LIMIT ?,?") ){
																	$cal_pageCat = ($current_pageCat - 1) * $rows_per_pageCat;
																	$statementCat->bind_param("ii",$cal_pageCat,$rows_per_pageCat);
																	$statementCat->execute();
																	$allbooksCat = $statementCat->get_result();
